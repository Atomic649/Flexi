...
const formatCurrency = (amount: number) => {
  ...
  return new Intl.NumberFormat('th-TH', { style: 'decimal', minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(amount) + ' THB';
};

// Format currency specifically for PDF to avoid Thai Baht symbol encoding issues
const formatCurrencyForPDF = (amount: number) => {
  return new Intl.NumberFormat('en-US', { style: 'decimal', minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(amount) + ' THB';
};
...
export default function Print() {
    ...
    try {
      ...
      if (response && response.length > 0) {
        const totalSales = response.reduce((sum: number, bill: any) => 
          sum + (Number(bill.price) * Number(bill.amount)), 0);
        
        const paidOrders = response.filter((bill: any) => bill.cashStatus).length;
  ...
  const searchByCustomerName = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await CallAPIPrint.searchBillsByCustomerAPI(memberId, searchQuery);
  ...
  const searchByDateRange = async (dates: string[]) => {
    ...
    try {
      ...
      if (dates.length === 1) {
        ...
        endDate = format(new Date(sortedDates[sortedDates.length - 1]), 'yyyy-MM-dd');
      }
      
      const response = await CallAPIPrint.getBillsByDateRangeAPI(memberId, startDate, endDate);
  ...
  const saveToPDF = async () => {
    ...
    try {
      ...
      drawText(t("print.salesTaxSummary"), width / 2, yPosition, { size: 16, font: boldFont, align: 'center' });
      yPosition -= lineHeight * 1.5;
      
      // Month and Year header
      const monthYearText = format(selectedMonth, 'MMMM yyyy');
      drawText(`${t("print.monthYear")} ${monthYearText}`, width / 2, yPosition, { size: 14, font: boldFont, align: 'center' });
      yPosition -= lineHeight * 2;
      
      // Company Information
      drawText(t("print.companyName"), margin, yPosition, { size: 12, font: boldFont });
      
      // Get business name from context or use placeholder
      const businessNameText = businessName || businessDetails?.businessName || "Your Business Name";
      drawText(businessNameText, margin + 120, yPosition, { size: 12 });
      
      // Tax ID on right side
      drawText(t("print.taxId"), width - margin - 200, yPosition, { size: 12, font: boldFont });
      yPosition -= lineHeight;
      
      // Company address
      if (businessDetails?.businessAddress) {
        drawText(t("print.address"), margin, yPosition, { size: 12, font: boldFont });
        drawText(businessDetails.businessAddress, margin + 120, yPosition, { size: 12 });
      }
      ...
      drawText(t("print.monthlySummary"), margin, yPosition, { size: 14, font: boldFont });
      yPosition -= lineHeight * 1.5;
      ...
      // Total Sales Data
      drawText(t("print.totalSales"), col1 + 5, yPosition - lineHeight + 5, { size: 11 });
      drawText(formatCurrencyForPDF(monthlyTotals.totalSales), col2 + 5, yPosition - lineHeight + 5, { size: 11 });
      
      // Calculate tax amount (7% standard VAT in Thailand)
      const taxAmount = monthlyTotals.totalSales * 0.07;
      drawText(formatCurrencyForPDF(taxAmount), col3 + 5, yPosition - lineHeight + 5, { size: 11 });
      ...
      // Total Orders
      drawText(t("print.totalOrders"), col1 + 5, yPosition - lineHeight + 5, { size: 11 });
      drawText(monthlyTotals.totalOrders.toString(), col2 + 5, yPosition - lineHeight + 5, { size: 11 });
      ...
      // Paid orders
      drawText(t("print.paidOrders"), col1 + 5, yPosition - lineHeight + 5, { size: 11 });
      drawText(`${monthlyTotals.paidOrders} (${monthlyTotals.totalOrders > 0 ? Math.round((monthlyTotals.paidOrders / monthlyTotals.totalOrders) * 100) : 0}%)`, 
        col2 + 5, yPosition - lineHeight + 5, { size: 11 });
      drawText("", col3 + 5, yPosition - lineHeight + 5, { size: 11 });
      ...
      // Unpaid orders
      drawText(t("print.unpaidOrders"), col1 + 5, yPosition - lineHeight + 5, { size: 11 });
      drawText(`${monthlyTotals.unpaidOrders} (${monthlyTotals.totalOrders > 0 ? Math.round((monthlyTotals.unpaidOrders / monthlyTotals.totalOrders) * 100) : 0}%)`, 
        col2 + 5, yPosition - lineHeight + 5, { size: 11 });
      drawText("", col3 + 5, yPosition - lineHeight + 5, { size: 11 });
      ...
      if (bills.length > 0) {
        drawText(t("print.invoiceList"), margin, yPosition, { size: 14, font: boldFont });
        yPosition -= lineHeight * 1.5;
        ...
        for (let i = 0; i < bills.length; i++) {
          ...
          if (itemCount >= itemsPerPage || yPosition < margin + lineHeight) {
            ...
            // Add page header
            drawText(t("print.invoiceList") + " " + t("print.continued"), width / 2, yPosition, { 
              size: 14, 
              font: boldFont,
              align: 'center'
            });
          ...
          // Draw row data
          // #
          drawText((i + 1).toString(), colStarts[0] + 3, yPosition - lineHeight + 5, { size: 10 });
          
          // Date
          drawText(formatDate(bill.purchaseAt), colStarts[1] + 3, yPosition - lineHeight + 5, { size: 10 });
          
          // Invoice No
          drawText(`#${bill.id}`, colStarts[2] + 3, yPosition - lineHeight + 5, { size: 10 });
          ...
          drawText(
            ...
            colStarts[3] + 3, yPosition - lineHeight + 5, { size: 10 }
          );
          
          // Payment status
          const statusColor = bill.cashStatus ? rgb(0, 0.5, 0) : rgb(0.8, 0.4, 0); // Green for paid, red for unpaid
          drawText(bill.cashStatus ? t("print.paid") : t("print.unpaid"), 
            colStarts[4] + 3, yPosition - lineHeight + 5, { size: 10, color: statusColor });
          
          // Price
          drawText(formatCurrencyForPDF(bill.price), colStarts[5] + 3, yPosition - lineHeight + 5, { size: 10 });
          
          // Total with VAT
          drawText(formatCurrencyForPDF(bill.price * bill.amount), colStarts[6] + 3, yPosition - lineHeight + 5, { size: 10 });
        ...
        // Totals text
        drawText(t("print.total"), margin + 3, yPosition - lineHeight + 5, { size: 10, font: boldFont });
        
        // Total amount
        const totalAmount = bills.reduce((sum, bill) => sum + (bill.price * bill.amount), 0);
        
        // Draw empty cells for date, invoice, customer
        // Draw the total amount in the price and total columns
        drawText(formatCurrencyForPDF(totalAmount), colStarts[5] + 3, yPosition - lineHeight + 5, { 
          size: 10,
          font: boldFont
        });
        
        drawText(formatCurrencyForPDF(totalAmount), colStarts[6] + 3, yPosition - lineHeight + 5, { 
          size: 10,
          font: boldFont
        });
      } else {
        // No invoices message
        drawText(t("print.noInvoicesFound"), margin, yPosition, { size: 12, font: italicFont });
      }
      
      // Add footer with generation date and page number
      const footerY = margin / 2;
      const footerText = `${t("print.generatedOn")} ${format(new Date(), 'dd/MM/yyyy HH:mm')} - ${t("print.page")} 1 - Flexi Business App`;
      drawText(footerText, width / 2, footerY, { size: 8, align: 'center', color: rgb(0.5, 0.5, 0.5) });
  ...
  const handleSavePdfToLocation = async (filePath: string) => {
    try {
      ...
      if (Platform.OS === 'web') {
        ...
        const newPath = `${destDir}income_report_${format(selectedMonth, 'yyyy-MM')}.pdf`;
  ...
  const renderInvoiceCard = (invoice: any) => {
    return (
      <TouchableOpacity
        key={invoice.id}
        className={`p-4 mb-3 rounded-lg border ${theme === 'dark' ? 'border-gray-700 bg-zinc-800' : 'border-gray-200 bg-white'}`}
        onPress={() => viewInvoiceDetails(invoice)}
      >
        <View className="flex-row justify-between items-center">
          <View>
            <CustomText weight="bold" className="mb-1 text-base">
              {t("print.invoice")} #{invoice.id}
            </CustomText>
            <CustomText className="mb-1">
              {invoice.cName} {invoice.cLastName}
            </CustomText>
            <CustomText className="mb-1">
          ...
          <View className="items-end">
            ...
            <View className={`mt-2 p-1 px-2 rounded-full ${invoice.cashStatus ? 'bg-green-700' : 'bg-orange-700'}`}>
              <CustomText className="text-white text-xs">
                {invoice.cashStatus ? t("bill.status.paid") : t("bill.status.unpaid")}
              </CustomText>
            </View>
          </View>
        </View>
              ...
              <View ref={printRef} className="p-4">
                <View className="flex-row justify-between items-center mb-6">
                  <CustomText weight="bold" className="text-xl">
                    {t("print.invoice")} #{selectedInvoice.id}
                  </CustomText>
                  <TouchableOpacity
                    onPress={() => setInvoiceModalVisible(false)}
                    className="p-2"
                ...
                <View className="flex-row justify-between mb-6">
                  <View>
                    <CustomText weight="bold" className="mb-1">{t("print.billedTo")}</CustomText>
                    <CustomText>{selectedInvoice.cName} {selectedInvoice.cLastName}</CustomText>
                    <CustomText>{selectedInvoice.cPhone}</CustomText>
                    <CustomText>{selectedInvoice.cAddress}</CustomText>
                    <CustomText>{selectedInvoice.cProvince}, {selectedInvoice.cPostId}</CustomText>
                  </View>
                  <View className="items-end">
                    <CustomText weight="bold" className="mb-1">{t("print.invoiceDate")}</CustomText>
                    <CustomText>{formatDate(selectedInvoice.purchaseAt)}</CustomText>
                    
                    <CustomText weight="bold" className="mb-1 mt-2">{t("print.paymentMethod")}</CustomText>
                    <CustomText>{selectedInvoice.payment}</CustomText>
                    
                    <CustomText weight="bold" className="mb-1 mt-2">{t("print.status")}</CustomText>
                    <View className={`p-1 px-2 rounded-full ${selectedInvoice.cashStatus ? 'bg-green-700' : 'bg-orange-700'}`}>
                      <CustomText className="text-white">
                        {selectedInvoice.cashStatus ? t("bill.status.paid") : t("bill.status.unpaid")}
                      </CustomText>
                    </View>
                  </View>
                </View>
                
                <View className={`p-4 rounded-lg ${theme === 'dark' ? 'bg-zinc-800' : 'bg-zinc-200'} mb-6`}>
                  <View className="flex-row justify-between mb-2">
                    <CustomText weight="bold">{t("print.productName")}</CustomText>
                  ...
                  <View className="flex-row justify-between">
                    ...
                    <CustomText>{formatCurrency(selectedInvoice.price)}</CustomText>
                    <CustomText>{formatCurrency(selectedInvoice.price * selectedInvoice.amount)}</CustomText>
                  </View>
                </View>
                ...
                <View className="flex-row justify-end">
                  <View className="w-1/2">
                    <View className="flex-row justify-between mb-2">
                      <CustomText weight="bold">{t("print.subtotal")}</CustomText>
                      <CustomText>{formatCurrency(selectedInvoice.price * selectedInvoice.amount)}</CustomText>
                    </View>
                    <View className="flex-row justify-between mb-2">
                      <CustomText weight="bold">{t("print.tax")} (7%)</CustomText>
                      <CustomText>{formatCurrency(selectedInvoice.price * selectedInvoice.amount * 0.07)}</CustomText>
                    </View>
                    <View className="flex-row justify-between mb-2">
                      <CustomText weight="bold">{t("print.grandTotal")}</CustomText>
                      <CustomText weight="bold">{formatCurrency(selectedInvoice.price * selectedInvoice.amount * 1.07)}</CustomText>
                    </View>
                  </View>
                </View>
      ...
      >
        ...
        >
          ...
          >
            ...
            {pdfPreviewUri && (
              <View className="mb-4">
                <Ionicons name="document-text" size={100} color="#0feac2" style={{ alignSelf: 'center' }} />
                <CustomText className="text-center text-zinc-500">
                  {t("print.pdfReady")}
                </CustomText>
              </View>
            )}
            ...
            <View className="flex-row justify-center">
              ...
              />
              <CustomButton
                title={t("print.savePdf")}
                handlePress={() => {
                  if (pdfPreviewUri) {
                    handleSavePdfToLocation(pdfPreviewUri);
                  }
                }}
                containerStyles="px-8"
                textStyles="!text-white"
      ...
      <ScrollView>
        ...
        >
          ...
          <View className="flex-row mb-6">
            ...
            >
              <CustomText
                weight={activeTab === TAB_INDICES.INDIVIDUAL_INVOICE ? "bold" : "regular"}
              >
            ...
            {activeTab === TAB_INDICES.MONTHLY_REPORT ? (
                ...
                {isLoading ? (
                  ...
                  <>
                    ...
                    <View className="flex-row flex-wrap mb-6">
                      <View 
                        className={`w-1/2 p-4 ${isMobile() ? 'w-full' : 'w-1/2 pr-2'} mb-4`}
                      >
                        <View className={`p-4 rounded-lg ${theme === 'dark' ? 'bg-zinc-700' : 'bg-zinc-200'}`}>
                      ...
                      <View 
                        className={`w-1/2 p-4 ${isMobile() ? 'w-full' : 'w-1/2 pl-2'} mb-4`}
                      >
                        <View className={`p-4 rounded-lg ${theme === 'dark' ? 'bg-zinc-700' : 'bg-zinc-200'}`}>
                          <CustomText className="text-zinc-500 mb-1">
                            {t("print.totalOrders")}
                      ...
                      <View 
                        className={`w-1/2 p-4 ${isMobile() ? 'w-full' : 'w-1/3 pr-2'} mb-4`}
                      >
                        <View className={`p-4 rounded-lg ${theme === 'dark' ? 'bg-zinc-700' : 'bg-zinc-200'}`}>
                          <CustomText className="text-zinc-500 mb-1">
                            {t("print.paidOrders")}
                      ...
                      <View 
                        className={`w-1/2 p-4 ${isMobile() ? 'w-full' : 'w-1/3 px-1'} mb-4`}
                      >
                        <View className={`p-4 rounded-lg ${theme === 'dark' ? 'bg-zinc-700' : 'bg-zinc-200'}`}>
                          <CustomText className="text-zinc-500 mb-1">
                          ...
                          <View className="flex-row items-center">
                            ...
                            <View className="ml-2 p-1 px-2 rounded-full bg-orange-700">
                              <CustomText className="text-white text-xs">
                                {monthlyTotals.totalOrders > 0
                                  ? `${Math.round((monthlyTotals.unpaidOrders / monthlyTotals.totalOrders) * 100)}%`
                                  : "0%"}
                              </CustomText>
                            </View>
                          </View>
                      ...
                      <View 
                        className={`w-1/2 p-4 ${isMobile() ? 'w-full' : 'w-1/3 pl-2'} mb-4`}
                      >
                        <View className={`p-4 rounded-lg ${theme === 'dark' ? 'bg-zinc-700' : 'bg-zinc-200'}`}>
                          <CustomText className="text-zinc-500 mb-1">
              ...
              <>
                ...
                <View className="flex-row mb-4 flex-wrap">
                  <View className={`${isMobile() ? 'w-full mb-4' : 'w-1/3 pr-2'}`}>
                    <Dropdown2
                      title={t("print.searchBy")}
                      options={[
                        { label: t("print.customerName"), value: SEARCH_TYPES.CUSTOMER_NAME },
                        { label: t("print.billId"), value: SEARCH_TYPES.BILL_ID },
                        { label: t("print.dateRange"), value: SEARCH_TYPES.DATE_RANGE },
                      ]}
                  ...
                  {searchType !== SEARCH_TYPES.DATE_RANGE && (
                    <View className={`${isMobile() ? 'w-full mb-4' : 'w-1/3 px-1'}`}>
                      <FormField2
                        title={searchType === SEARCH_TYPES.CUSTOMER_NAME ? t("print.enterCustomerName") : t("print.enterBillId")}
                        value={searchQuery}
                        handleChangeText={setSearchQuery}
                        placeholder={searchType === SEARCH_TYPES.CUSTOMER_NAME ? t("bill.enterName") : "123"}
                        bgColor={theme === "dark" ? "#2D2D2D" : "#e1e1e1"}
                        placeholderTextColor={theme === "dark" ? "#606060" : "#b1b1b1"}
                        textcolor={theme === "dark" ? "#b1b1b1" : "#606060"}
                        keyboardType={searchType === SEARCH_TYPES.BILL_ID ? "numeric" : "default"}
                      />
                    </View>
                  )}
                  
                  <View className={`${isMobile() ? 'w-full' : 'w-1/3 pl-2'} flex justify-end`}>
                    <CustomButton
                ...
                {isLoading ? (
                  ...
                  <>
                    {searchType === SEARCH_TYPES.DATE_RANGE && dateRange.length > 0 && (
                    ...
                    {bills.length > 0 ? (
                      <>
                        <CustomText weight="bold" className="mb-4">
                          {t("print.searchResults")} ({bills.length})
                        </CustomText>
                        {bills.map((invoice) => renderInvoiceCard(invoice))}
                      </>
                    ) : (

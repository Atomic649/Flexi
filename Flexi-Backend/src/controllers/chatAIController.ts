...
export async function chatAIWithLangChain(req: Request, res: Response) {
  ...
  try {
    const { prompt, messages, sessionId: inputSessionId, userId, system, temperature } = req.body || {};

    // Resolve member uniqueId to attach session; if not provided, try from token's user id
    let memberId: string | undefined = typeof userId === "string" ? userId : undefined;
    if (!memberId) {
    ...
    if (!memberId) return res.status(400).json({ error: "Missing userId (member uniqueId)" });

    // Ensure a session
    let sessionId = inputSessionId as string | undefined;
    if (!sessionId) {
      const session = await prisma.chatSession.create({ data: { userId: memberId } });
    ...
    let userContent = "";
    if (typeof prompt === "string" && prompt.trim()) userContent = prompt.trim();
    else if (Array.isArray(messages) && messages.length > 0) {
      const last = messages[messages.length - 1];
      if (last?.role === "user" && typeof last?.content === "string") userContent = last.content;
    }
    if (!userContent) return res.status(400).json({ error: "Provide prompt or messages[] ending with user" });

    // Store user message
    await prisma.chatMessage.create({
      data: { sessionId, message: { role: "user", content: userContent } as any },
    });
    ...
    if (userMsgCount >= 3 && !session?.title) {
      ...
      const titleText = typeof (titleResp as any).content === "string" ? (titleResp as any).content : Array.isArray((titleResp as any).content) ? (titleResp as any).content.map((c: any) => c?.text || c?.content || "").join(" ") : "";
      const trimmedTitle = (titleText || "Session").replace(/^["'\s]+|["'\s]+$/g, "").slice(0, 60);
      await prisma.chatSession.update({ where: { id: sessionId }, data: { title: trimmedTitle } });
    }
    ...
    const summaryResp = await llm.invoke([
      { role: "system", content: "You summarize chats for records. Be concise and neutral." },
      { role: "user", content: summaryPrompt },
    ] as any);
    const summaryText = typeof (summaryResp as any).content === "string" ? (summaryResp as any).content : Array.isArray((summaryResp as any).content) ? (summaryResp as any).content.map((c: any) => c?.text || c?.content || "").join(" ") : "";
      await prisma.chatSession.update({ where: { id: sessionId }, data: { summary: summaryText.slice(0, 2000) } });

  // Very short title from summary after >=3 total messages
  const totalMsgCount = await prisma.chatMessage.count({ where: { sessionId } });
  const sessionAfterSummary = await prisma.chatSession.findUnique({ where: { id: sessionId } });
  if (totalMsgCount >= 3 && !sessionAfterSummary?.title) {
        ...
        const titleText = typeof (titleResp as any).content === "string" ? (titleResp as any).content : Array.isArray((titleResp as any).content) ? (titleResp as any).content.map((c: any) => c?.text || c?.content || "").join(" ") : "";
        const trimmedTitle = (titleText || "Session").replace(/^["'\s]+|["'\s]+$/g, "").slice(0, 40);
        await prisma.chatSession.update({ where: { id: sessionId }, data: { title: trimmedTitle } });
      }
...
export async function chatAIStreamWithLangChain(req: Request, res: Response) {
  ...
  try {
    ...
    // Store user message
    await prisma.chatMessage.create({ data: { sessionId, message: { role: "user", content: userContent } as any } });
    ...
    const session = await prisma.chatSession.findUnique({ where: { id: sessionId } });
    if (userMsgCount >= 3 && !session?.title) {
      const convo = history.map((m) => `${(m.message as any).role}: ${(m.message as any).content}`).join("\n");
      const titleResp = await llm.invoke([
        { role: "system", content: "You generate concise conversation titles (<= 6 words)." },
        { role: "user", content: `Write a short, specific title for this conversation, no quotes.\n\n${convo}` },
      ] as any);
      const ttxt = typeof (titleResp as any).content === "string" ? (titleResp as any).content : Array.isArray((titleResp as any).content) ? (titleResp as any).content.map((c: any) => c?.text || c?.content || "").join(" ") : "";
      const trimmed = (ttxt || "Session").replace(/^["'\s]+|["'\s]+$/g, "").slice(0, 60);
      await prisma.chatSession.update({ where: { id: sessionId }, data: { title: trimmed } });
      send({ title: trimmed });
    }
    ...
    await prisma.chatSession.update({ where: { id: sessionId }, data: { summary: stext.slice(0, 2000) } });
      ...
      const totalMsgCount2 = await prisma.chatMessage.count({ where: { sessionId } });
      const session2 = await prisma.chatSession.findUnique({ where: { id: sessionId } });
      if (totalMsgCount2 >= 3 && !session2?.title) {
        ...
        const t2 = typeof (titleResp2 as any).content === "string" ? (titleResp2 as any).content : Array.isArray((titleResp2 as any).content) ? (titleResp2 as any).content.map((c: any) => c?.text || c?.content || "").join(" ") : "";
  ...
  try { send({ error: err?.message || "AI error" }); } catch {}
  clearInterval(heartbeat);
  res.end();
  }
}
